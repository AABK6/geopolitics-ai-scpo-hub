...
const initLatentSpace = () => {
  ...
  beginBtn.innerText = 'Begin Course Flythrough';
  beginBtn.style.position = 'absolute';
  beginBtn.style.bottom = '3rem';
  beginBtn.style.left = '50%';
  beginBtn.style.transform = 'translateX(-50%)';
  beginBtn.style.padding = '1rem 2rem';
  beginBtn.style.background = '#ea580c';
  beginBtn.style.color = 'white';
  beginBtn.style.border = 'none';
  beginBtn.style.borderRadius = '4px';
  beginBtn.style.fontFamily = 'JetBrains Mono, monospace';
  beginBtn.style.textTransform = 'uppercase';
  beginBtn.style.cursor = 'pointer';
  beginBtn.style.zIndex = '100';
  beginBtn.style.pointerEvents = 'auto';
  document.getElementById('ui-layer').appendChild(beginBtn);
  ...
  beginBtn.addEventListener('click', () => {
    ...
    // Find node 1
    const node1 = meshes.find(m => m.userData.id === 'm1');
    if (node1) {
      // Fly camera
      gsap.to(camera.position, {
        x: node1.position.x + 3,
        y: node1.position.y + 2,
        z: node1.position.z + 5,
        duration: 3,
        ease: 'power3.inOut',
