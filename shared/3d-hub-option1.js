...
const initLatentSpace = () => {
  ...
  const nodesData = [
    ...
    },
    {
      ...
      position: new THREE.Vector3(12, 3, 2),
      color: 0x38bdf8,
      size: 0.8
    },
    {
      ...
      position: new THREE.Vector3(-12, -1, 3),
      color: 0x38bdf8,
      size: 0.8
    },
    {
    ...
    },
    {
      ...
      position: new THREE.Vector3(17, -4, -2),
      color: 0x38bdf8,
      size: 0.8
    }
  ];
  ...
  // Create a SPLINE CURVE for anchors to emphasize progression
  const anchors = nodesData.filter(n => n.type === 'anchor');
  const anchorPoints = anchors.map(a => a.position);
  const spline = new THREE.CatmullRomCurve3(anchorPoints);
  const tubeGeometry = new THREE.TubeGeometry(spline, 64, 0.1, 8, false);
  const tubeMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.2 });
  const tubeMesh = new THREE.Mesh(tubeGeometry, tubeMaterial);
  constellation.add(tubeMesh);
